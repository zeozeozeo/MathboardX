...
git_heads = {
	...
	"dear_bindings": "c8049278a3dcda45dea93555534c8cab250d566b",
}
...
wanted_backends = ["glfw", "webgl", "opengl3"]
...
def compile(backend_deps_names: typing.Set[str], all_sources: typing.List[str], wasm: bool):
	...
	if wasm:
		...
		libc_include_path = os.path.join(root, "vendor", "libc-shim", "include")
		...
		compile_flags.append(f"-isystem{libc_include_path}")
	else:
	...
	# See did_re_execute
	if wasm:
		exec(["clang"] + compile_flags + ["-c"] + all_sources, "Compiling WASM sources")
...
def main():
	...
	if platform.system() == "Windows":
		if build_wasm:
			assertx(has_tool("clang"), "clang is required for WASM builds on Windows. Please install LLVM/Clang.")
