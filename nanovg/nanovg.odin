...
__setDevicePxRatio :: proc(ctx: ^Context, ratio: f32) {
	...
	ctx.distTol = 0.0001 / ratio // MATHBOARDX: changed from 0.01
...
TransformPoint :: proc(dx: ^f32, dy: ^f32, t: Matrix, sx: f32, sy: f32) {
...
CreateImageRGBA :: proc(ctx: ^Context, w, h: int, imageFlags: ImageFlags, data: []byte) -> int {
	assert(ctx.params.renderCreateTexture != nil)
	return ctx.params.renderCreateTexture(ctx.params.userPtr, .RGBA, w, h, imageFlags, data)
}
...
*/
LinearGradient :: proc(sx, sy: f32, ex, ey: f32, icol: Color, ocol: Color) -> (p: Paint) {
	LARGE :: f32(1e5)
...
/*
	...
	The gradient is transformed by the current transform when it is passed to nvgFillPaint() or nvgStrokePaint().
*/
RadialGradient :: proc(cx, cy: f32, inr: f32, outr: f32, icol: Color, ocol: Color) -> (p: Paint) {
	r := (inr + outr) * 0.5
	f := (outr - inr)
...
/*
	...
	The gradient is transformed by the current transform when it is passed to nvgFillPaint() or nvgStrokePaint().
*/
BoxGradient :: proc(x, y: f32, w, h: f32, r: f32, f: f32, icol: Color, ocol: Color) -> (p: Paint) {
	TransformIdentity(&p.xform)
	p.xform[4] = x + w * 0.5
	p.xform[5] = y + h * 0.5
...
IntersectScissor :: proc(ctx: ^Context, x, y, w, h: f32) {
	isect_rects :: proc(dst: ^[4]f32, ax, ay, aw, ah: f32, bx, by, bw, bh: f32) {
		minx := max(ax, bx)
		miny := max(ay, by)
...
GlobalCompositeBlendFuncSeparate :: proc(
	...
	state := __getState(ctx)
	state.compositeOperation = CompositeOperationState{srcRGB, dstRGB, srcAlpha, dstAlpha}
}
...
__ptEquals :: proc(x1, y1, x2, y2, tol: f32) -> bool {
	dx := x2 - x1
	dy := y2 - y1
	// MATHBOARDX: added epsilon
	return dx * dx + dy * dy < (tol * tol) + 1e-11
}
...
__addPath :: proc(ctx: ^Context) {
	append(&ctx.cache.paths, Path{first = len(ctx.cache.points), winding = .CCW})
}
...
__addPoint :: proc(ctx: ^Context, x, y: f32, flags: PointFlags) {
	...
	append(&ctx.cache.points, Point{x = x, y = y, flags = flags})
	path.count += 1
}
...
__flattenPaths :: proc(ctx: ^Context) {
	...
	for &path in cache.paths {
		if path.count <= 0 do continue // MATHBOARDX: safety
		...
		if path.count > 1 {
			p0 := &pts[path.count - 1]
			p1 := &pts[0]
			if __ptEquals(p0.x, p0.y, p1.x, p1.y, ctx.distTol) { 	// MATHBOARDX: safety: only attempt to close if we have at least 2 points
				path.count -= 1
				path.closed = true
			}
		}
		...
		if path.count > 2 {
			...
			if path.winding == .CCW && area < 0 do __polyReverse(pts[:path.count])
			if path.winding == .CW && area > 0 do __polyReverse(pts[:path.count])
		}
...
__chooseBevel :: proc(bevel: bool, p0: ^Point, p1: ^Point, w: f32, x0, y0, x1, y1: ^f32) {
	if bevel {
		x0^ = p1.x + p0.dy * w
		y0^ = p1.y - p0.dx * w
		x1^ = p1.x + p1.dy * w
...
__roundJoin :: proc(dst: ^[]Vertex, p0, p1: ^Point, lw, rw: f32, lu, ru: f32, ncap: int) {
...
__bevelJoin :: proc(dst: ^[]Vertex, p0, p1: ^Point, lw, rw: f32, lu, ru: f32) {
	dlx0, dly0 := p0.dy, -p0.dx
	dlx1, dly1 := p1.dy, -p1.dx
...
__calculateJoins :: proc(ctx: ^Context, w: f32, lineJoin: LineCapType, miterLimit: f32) {
	cache := &ctx.cache
	iw := f32(0)
...
__expandStroke :: proc(
	...
	u0 := f32(0.0)
	u1 := f32(1.0)
	// MATHBOARDX: safety; changed from:
	//ncap := __curveDivs(w, math.PI, ctx.tessTol)	// Calculate divisions per half circle.
	ncap_f := __curveDivs(w, math.PI, ctx.tessTol)
	ncap := int(clamp(ncap_f, 2, 30))
...
__expandFill :: proc(ctx: ^Context, w: f32, lineJoin: LineCapType, miterLimit: f32) -> bool {
	cache := &ctx.cache
	aa := ctx.fringeWidth
	fringe := w > 0.0
	__calculateJoins(ctx, w, lineJoin, miterLimit)
...
BezierTo :: proc(ctx: ^Context, c1x, c1y: f32, c2x, c2y: f32, x, y: f32) {
...
ArcTo :: proc(ctx: ^Context, x1, y1: f32, x2, y2: f32, radius: f32) {
...
RoundedRectVarying :: proc(
	...
	if radius_top_left < 0.1 &&
	   radius_top_right < 0.1 &&
	   radius_bottom_right < 0.1 &&
	   radius_bottom_left < 0.1 {
		Rect(ctx, x, y, w, h)
	} else {
		halfw := abs(w) * 0.5
		halfh := abs(h) * 0.5
...
Stroke :: proc(ctx: ^Context) {
	...
	// MATHBOARDX: changed from
	//strokeWidth := clamp(state.strokeWidth * scale, 0, 200)
	strokeWidth := clamp(state.strokeWidth * scale, 0, 1_000_000)
...
AddFallbackFont :: proc(ctx: ^Context, base_font: string, fallback_font: string) -> bool {
	return AddFallbackFontId(ctx, FindFont(ctx, base_font), FindFont(ctx, fallback_font))
}
...
//
// MATHBOARDX: baking additions
//

BakedPathMeta :: struct {
	stroke_offset: int,
	stroke_count:  int,
	closed:        bool,
	convex:        bool,
...
BakedStroke :: struct {
	verts: [dynamic]Vertex,
	paths: [dynamic]BakedPathMeta,
}

DeleteBakedStroke :: proc(baked: ^BakedStroke) {
...
// Bake whatever is in the current commnd buffer
// Note: the caller must ensure Transform is set to Identity before adding MoveTo/LineTo commands for world-space baking
BakeStroke :: proc(ctx: ^Context, translation_offset: [2]f64 = {0, 0}) -> (baked: BakedStroke) {
	state := __getState(ctx)

	__flattenPaths(ctx)

	// project screen space to world space
	inv_xform: Matrix
	if !TransformInverse(&inv_xform, state.xform) {
		TransformIdentity(&inv_xform)
	}

	baked.verts = make([dynamic]Vertex, len(ctx.cache.verts))

	// convert verts from screen space to world space
	for v, i in ctx.cache.verts {
		tx := v.x * inv_xform[0] + v.y * inv_xform[2] + inv_xform[4]
		ty := v.x * inv_xform[1] + v.y * inv_xform[3] + inv_xform[5]

		final_x := tx + f32(translation_offset.x)
		final_y := ty + f32(translation_offset.y)

		baked.verts[i] = {final_x, final_y, v[2], v[3]}
	}

	baked.paths = make([dynamic]BakedPathMeta, len(ctx.cache.paths))
	for p, i in ctx.cache.paths {
		if len(p.stroke) > 0 {
			start_ptr := raw_data(p.stroke)
			base_ptr := raw_data(ctx.cache.verts)
			offset := (uintptr(start_ptr) - uintptr(base_ptr)) / size_of(Vertex)

			baked.paths[i] = {
				stroke_offset = int(offset),
				stroke_count  = len(p.stroke),
				closed        = p.closed,
				convex        = p.convex,
			}
		}
...
// Draw baked vertices with transform
StrokeBaked :: proc(ctx: ^Context, baked: ^BakedStroke) {
	if len(baked.verts) == 0 do return
	...
	start_vert_idx := len(ctx.cache.verts)
	resize(&ctx.cache.verts, start_vert_idx + len(baked.verts))

	// project world space -> screen space
	xform := state.xform
	src := baked.verts[:]
	dst := ctx.cache.verts[start_vert_idx:]

	for i in 0 ..< len(src) {
		v := src[i]
		tx := v.x * xform[0] + v.y * xform[2] + xform[4]
		ty := v.x * xform[1] + v.y * xform[3] + xform[5]
		dst[i] = {tx, ty, v[2], v[3]}
	}

	start_path_idx := len(ctx.cache.paths)
	for meta in baked.paths {
		current_offset := start_vert_idx + meta.stroke_offset
		p := Path {
			closed = meta.closed,
			convex = meta.convex,
			stroke = ctx.cache.verts[current_offset:current_offset + meta.stroke_count],
		}
		append(&ctx.cache.paths, p)
	}

	paths_to_draw := ctx.cache.paths[start_path_idx:]
	...
	ctx.params.renderStroke(
		...
		&state.scissor,
		ctx.fringeWidth,
		state.strokeWidth * __getAverageScale(state.xform[:]),
		paths_to_draw,
	)

	resize(&ctx.cache.paths, start_path_idx)
	resize(&ctx.cache.verts, start_vert_idx)
}
